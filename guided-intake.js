(() => {
  ...
  function syncDirectiveQuestions(mode = selectedDirective) {
    ...
    const pairs = mode === "buy"
      ...
      : [["intakeSellAsk","sellAsk"],["intakeSellFloor","sellFloor"],["intakeSellCosts","sellCosts"],["intakeSellReconMode","sellReconMode"]];
    ...
    shell.innerHTML = `
      ...
      <div class="intake-step" data-intake-step="3">
        ...
        <div id="directiveQuestions" class="directive-questions hidden">
          ...
          <div id="sellDirectiveQuestions" class="directive-question-set hidden"><div class="directive-question-head"><b>Selling details</b><span>Enter the actual asking price and minimum acceptable price. The assessment will calculate what the vehicle is worth.</span></div><div class="grid3 field-grid"><label>Asking Price <span class="v1214-field-badge required">Required</span><input id="intakeSellAsk" inputmode="decimal"><span class="field-help">The price the vehicle is advertised or planned to be advertised for—not the app's value estimate.</span></label><label>Minimum Acceptable Price <span class="v1214-field-badge required">Required</span><input id="intakeSellFloor" inputmode="decimal"><span class="field-help">The lowest sale price you are willing to accept.</span></label><label>Known Selling Costs <span class="v1214-field-badge optional">Optional</span><input id="intakeSellCosts" inputmode="decimal"><span class="field-help">Advertising, detailing, transport, paperwork or other known sale costs.</span></label><label>Recon Strategy <span class="v1214-field-badge optional">Optional</span><select id="intakeSellReconMode"><option value="all">Complete Planned Recon</option><option value="required">Required Only</option><option value="none">Sell As-Is</option></select><span class="field-help">Choose how much planned work should be included in the selling analysis.</span></label></div></div>
        </div>
